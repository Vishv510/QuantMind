...
import com.stock.portfolio.entity.*;
import com.stock.portfolio.repository.*;
...
public class PortfolioService {
 ...
 @Transactional public PortfolioResponse buy(TradeRequest r){
        String s = symbol(r.symbol());
        BigDecimal q = scale(r.quantity());
        BigDecimal price = scale(r.price());
        Optional<Portfolio> existing = portfolios.findByUserIdAndSymbol(r.userId(), s);
        Portfolio p = existing.orElseGet(() -> Objects.requireNonNull(portfolios.save(Objects.requireNonNull(Portfolio.create(r.userId(), s, q, price)))));
        if (existing.isPresent()) {
            p.buy(q, price);
        }
        transactions.save(Objects.requireNonNull(Transaction.create(p, TransactionType.BUY, q, price, BigDecimal.ZERO.setScale(4))));
        return response(p);
    }
 @Transactional public PortfolioResponse sell(TradeRequest r) {
        Portfolio p = find(r.userId(), symbol(r.symbol()));
        BigDecimal pnl = p.sell(scale(r.quantity()), scale(r.price()));
        transactions.save(Objects.requireNonNull(Transaction.create(p, TransactionType.SELL, scale(r.quantity()), scale(r.price()), pnl)));
        return response(p);
    }
